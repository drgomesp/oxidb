    ...
    fn iter<'b>(&'b self) -> Box<dyn Iterator<Item = Cow<'b, [ColumnValue]>> + 'b> {
        ...
        Box::new(self.offsets.iter().map(move |offset| {
            ...
            let row: Vec<ColumnValue> = self
                ...
                .iter()
                .map(|column| {
                    let size = match column.get_data_type().get_fixed_length() {
                        Some(l) => l as usize,
                        None => 32,
                    };

                    let bytes = &self.data[column_offset..column_offset as usize + size];

                    column_offset += size;

                    ColumnValueOps::from_bytes(column.get_data_type(), Cow::from(bytes))
        ...
        T: Iterator<Item = &'b Box<[u8]>>,
    {
        ...
        for (column, data_box) in self.columns.iter().zip(row) {
            ...
            row_size += column
                .get_data_type()
                .get_fixed_length()
                .expect("could not get column fixed length");
        }
...
impl ColumnValueOps for ColumnValue {
    ...
    fn to_bytes(&self, column_type: &Self::ColumnType) -> Result<Box<[u8]>, Error> {
        match (&self, column_type) {
            (ColumnValue::StringLiteral(ref s), DataType::String { .. }) => {
                Ok(s.to_owned().into_boxed_str().into_boxed_bytes())
            }
    ...
    values
        .map(|value| match value {
            ColumnValue::StringLiteral(ref s) => {
                value.to_bytes(&DataType::String { len: s.len() }).unwrap()
            }
